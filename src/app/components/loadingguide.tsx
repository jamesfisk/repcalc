...
function LoadingGuide(props: LoadingGuideProps) {
  ...
  const load: any = iskg
    ? getKgLoads(props.weight, useCollars)
    : getLbsLoads(props.weight);
  ...
  const redstyles: SxProps =
    (iskg && load.twentyfives > 0) || (!iskg && load.fortyfives > 0)
      ? {
          ...baseStyles,
      ...
      : baseStyles;
  const bluestyles: SxProps =
    (iskg && load.twenties > 0) || (!iskg && load.thirtyfives > 0)
      ? {
          ...baseStyles,
      ...
      : baseStyles;
  const yellowstyles: SxProps =
    (iskg && load.fifteens > 0) || (!iskg && load.twentyfives > 0)
      ? {
          ...baseStyles,
  ...
  const whitestyles: SxProps =
    load.fives > 0
      ...
      : baseStyles;
  const blackstyles: SxProps =
    load.twopointfives > 0
      ? {
          ...baseStyles,
      ...
      : baseStyles;
  const silverstyles: SxProps =
    load.onepointtwofives > 0
      ? {
          ...baseStyles,
    ...
    return (
      ...
      >
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox checked={useCollars} onChange={handleCollarsClick} />
            }
            label="2.5kg collars"
          />
        </FormGroup>
  ...
  return (
    <div
      className={`w-full pt-4 flex ${props.forceStackUi ? "" : "md:flex-row md:justify-around"} flex-col justify-center`}
    >
          ...
          <Paper sx={{ width: "100%", mb: 2 }}>
            ...
            <TableContainer sx={{ width: "100%" }}>
              <Table sx={{ width: "100%" }} aria-label="simple table">
                <TableHead>
                  <TableRow>
                    <TableCell align="center" sx={redstyles} padding="none">
                      {iskg ? "25s" : "45s"}
                    </TableCell>
                    <TableCell align="center" sx={bluestyles} padding="none">
                      {iskg ? "20s" : "35s"}
                    </TableCell>
                    <TableCell align="center" sx={yellowstyles} padding="none">
                      {iskg ? "15s" : "25s"}
                    </TableCell>
                    <TableCell align="center" sx={greenstyles} padding="none">
                      10s
                    </TableCell>
                    <TableCell align="center" sx={whitestyles} padding="none">
                      5s
                    </TableCell>
                    <TableCell align="center" sx={blackstyles} padding="none">
                      2.5s
                    </TableCell>
                    <TableCell align="center" sx={silverstyles} padding="none">
                      1.25s
                    </TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  ...
                  >
                    <TableCell align="center" sx={redstyles} padding="none">
                      {iskg ? load.twentyfives : load.fortyfives}
                    </TableCell>
                    <TableCell align="center" sx={bluestyles} padding="none">
                      {iskg ? load.twenties : load.thirtyfives}
                    </TableCell>
                    <TableCell align="center" sx={yellowstyles} padding="none">
                      {iskg ? load.fifteens : load.twentyfives}
                    </TableCell>
                    <TableCell align="center" sx={greenstyles} padding="none">
                      {load.tens}
                    </TableCell>
